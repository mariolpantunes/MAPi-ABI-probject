...
{
    min.value <- (-1*min(ts.data))+10

    ts.data.processed <- log((ts.data+min.value))
...
# Max error
me <- function(preds, trues)
{
    error <- abs(preds-trues)
    error <- as.vector(na.omit(error))
...
error.func <- function(preds, test.data, learner.pars)
{
    ...
    me = me(preds,test.data[,target.name(form,test.data)]))
}
...
# Load data from CSV
raw.data <- read.csv("data3.csv", header = TRUE, stringsAsFactors = FALSE)
